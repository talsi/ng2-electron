...
export class JsonSchemaFormService {
  ...
  public removeArrayItem(ctx: any, index: number): boolean {
    if (!ctx.layoutNode || !ctx.layoutNode.dataPointer || !ctx.dataIndex ||
      !ctx.layoutNode.layoutPointer || !ctx.layoutIndex ||
      !isDefined(index) || !isDefined(index)) { return false; }

    // Move item in the formArray
    let formArray = <FormArray>this.getControlGroup(ctx).controls[ctx.layoutNode.name];
    formArray.controls.splice(index, 1);
    formArray.updateValueAndValidity();
    (<any>formArray)._onCollectionChange();

    // Move layout item
    let layoutArray = ctx.layoutNode.items;
